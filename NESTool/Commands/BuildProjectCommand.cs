...
namespace NESTool.Commands
{
    public class BuildProjectCommand : Command
    {
        ...
        private void BuildBackgrounds()
        {
            ...
            foreach (FileModelVO item in models)
            {
                ...
                using (StreamWriter outputFile = new StreamWriter(fullPath))
                {
                    ...
                    {
                        WriteNametableCompresed(outputFile, model, item.Name);
                    }
                    else
                    {
        ...
        private void WriteNametableCompresed(StreamWriter outputFile, MapModel model, string name)
        {
            ...
            for (int j = 0; j < 15; ++j)
            {
                ...
                for (int k = 0; k < 2; ++k)
                {
                    ...
                    for (int i = 0; i < 16; ++i)
                    {
                        ...
                        SerializeMapTile(ref serializedMap, mapTile[0 + (k * 2)]);
                        SerializeMapTile(ref serializedMap, mapTile[1 + (k * 2)]);
                    }
                }
            }
            ...
            {
                if (i % 32 == 0 && i < compressedData.Count - 1)
                {
                    if (i > 0)
                    {
                        outputFile.Write(Environment.NewLine);
        ...
        private void SerializeMapTile(ref List<byte> serializedData, MapTile mapTile)
        {
            BankModel bank = ProjectFiles.GetModel<BankModel>(mapTile.BankID);
